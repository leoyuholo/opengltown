...
void Road::draw(DrawingState*)
{
  ...
  glColor3f(.6f,.6f,.6f);
  glNormal3f(0,1,0);
  fetchTexture("rock055.png",true,true);
  roadDraw();
...
/**********************added by ding ******************************/
EllipticalRoad::EllipticalRoad(float cx_, float cz_, float rx_, float rz_) :
	Road(cx+rx,cz,cx+rx,cz),
	cx(cx_), cz(cz_), rx(rx_), rz(rz_)
{
}

void EllipticalRoad::roadDraw()
{
	...
	for(float u=0; u<=1.01; u+=.01f) {
		...
		glVertex3f(cx + c * (rx+roadWidth), 0, cz + s * (rz+roadWidth));
		glVertex3f(cx + c * (rx-roadWidth), 0, cz + s * (rz-roadWidth));
	}
	glEnd();
}

void EllipticalRoad::position(int lane, float u, float& x, float& z, float& dx, float& dz)
{
	float lrx = rx + static_cast<float>((lane) ? roadWidth2 : -roadWidth2);
	float lrz = rz + static_cast<float>((lane) ? roadWidth2 : -roadWidth2);
